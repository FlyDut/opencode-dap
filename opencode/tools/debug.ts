...
function resolveDisassemblyReference(
	memoryReference: string | undefined,
): string {
	const session = dapSessionManager.getActiveSession();
	return (
		memoryReference ??
		session?.instructionPointerReference ??
		""
	);
}
...
function requireCapability(
	...
	const capabilities = dapSessionManager.getCapabilities();
...
function formatSessionSnapshot(snapshot: DapSessionSummary): string[] {
	...
	if (snapshot.status === "stopped") {
		if (snapshot.stopReason) lines.push(`Stop reason: ${snapshot.stopReason}${snapshot.stopDescription ? ` (${snapshot.stopDescription})` : ""}`);
...
function formatInstructionBreakpoints(
	bps: DapInstructionBreakpointRecord[],
): string {
	...
	if (bps.length === 0) {
		...
		for (const bp of bps) {
			const status = bp.verified ? "✓" : "✗";
			lines.push(`  ${status} ${bp.instructionReference}${bp.offset !== undefined ? `+${bp.offset}` : ""}`);
		}
	}
	return lines.join("\n");
}
...
function formatDataBreakpointInfo(info: DapDataBreakpointInfoResponse): string {
	const lines = [`Data ID: ${info.dataId ?? "(not supported)"}`, `Description: ${info.description}`];
...
function formatSessions(
	...
	return sessions.map(s => `  [${s.id}] ${s.adapter} - ${s.status}${s.program ? ` - ${s.program}` : ""}`).join("\n");
}
...
Execution actions (take care): launch, attach, set_breakpoint, remove_breakpoint, continue, step_over, step_in, step_out, pause, evaluate, terminate, write_memory, custom_request`,
	...
	args: {
		...
		program: tool.schema
			.string()
			.optional()
			.describe("Path to program or script to debug (for launch)"),
		args: tool.schema
			.array(tool.schema.string())
			.optional()
			.describe("Command-line arguments to pass to the debugged program"),
		cwd: tool.schema
			.string()
			.optional()
			.describe("Working directory for the debug session (defaults to project root)"),
		adapter: tool.schema
			.string()
			.optional()
			.describe("Debug adapter name (auto-selected if omitted)"),
		pid: tool.schema
			.number()
			.optional()
			.describe("Process ID to attach to"),
		port: tool.schema
			.number()
			.optional()
			.describe("Port to attach to"),
		host: tool.schema
			.string()
			.optional()
			.describe("Host for TCP attach (default: localhost)"),
		file: tool.schema
			.string()
			.optional()
			.describe("Source file path for file-based breakpoints"),
		line: tool.schema
			.number()
			.optional()
			.describe("Line number for file-based breakpoints"),
		function: tool.schema
			.string()
			.optional()
			.describe("Function name for function breakpoints"),
		condition: tool.schema
			.string()
			.optional()
			.describe("Conditional expression for breakpoints"),
		...
		context: tool.schema
			.string()
			.optional()
			.describe("Evaluation context: watch, repl, hover, clipboard, variables"),
		frame_id: tool.schema
			.number()
			.optional()
			.describe("Stack frame ID for scoped operations"),
		levels: tool.schema
			.number()
			.optional()
			.describe("Number of stack frames to fetch"),
		variable_ref: tool.schema
			.number()
			.optional()
			.describe("Variable reference for child variables"),
		scope_id: tool.schema
			.number()
			.optional()
			.describe("Scope reference for variables"),
		instruction_reference: tool.schema
			.string()
			.optional()
			.describe("Instruction pointer reference"),
		offset: tool.schema
			.number()
			.optional()
			.describe("Byte offset for memory/instruction operations"),
		instruction_offset: tool.schema
			.number()
			.optional()
			.describe("Instruction offset for disassembly"),
		instruction_count: tool.schema
			.number()
			.optional()
			.describe("Number of instructions to disassemble"),
		resolve_symbols: tool.schema
			.boolean()
			.optional()
			.describe("Resolve symbols in disassembly"),
		memory_reference: tool.schema
			.string()
			.optional()
			.describe("Memory reference for read/write"),
		count: tool.schema
			.number()
			.optional()
			.describe("Byte count for memory read"),
		data: tool.schema
			.string()
			.optional()
			.describe("Data to write (base64 for memory, string for expressions)"),
		allow_partial: tool.schema
			.boolean()
			.optional()
			.describe("Allow partial memory writes"),
		name: tool.schema
			.string()
			.optional()
			.describe("Variable name for data breakpoint info"),
		data_id: tool.schema
			.string()
			.optional()
			.describe("Data ID for data breakpoints"),
		access_type: tool.schema
			.enum(["read", "write", "readWrite"] as const)
			.optional()
			.describe("Access type for data breakpoints"),
		hit_condition: tool.schema
			.string()
			.optional()
			.describe("Hit condition for breakpoints"),
		start_module: tool.schema
			.number()
			.optional()
			.describe("Start module index"),
		module_count: tool.schema
			.number()
			.optional()
			.describe("Number of modules to fetch"),
		command: tool.schema
			.string()
			.optional()
			.describe("Custom DAP command name"),
		arguments: tool.schema
			.record(tool.schema.unknown())
			.optional()
			.describe("Custom DAP command arguments"),
		timeout: tool.schema
			.number()
			.optional()
			.default(30)
	...
	async execute(args, context) {
		...
		const cwd = typeof args.cwd === "string" && args.cwd ? args.cwd : context.directory ?? process.cwd();
		...
		switch (args.action) {
			...
			case "attach": {
				...
				const snapshot = await dapSessionManager.attach({ adapter, cwd, pid, port, host }, undefined, timeoutMs);
				return formatSessionSnapshot(snapshot).join("\n");
			}
			case "terminate": {
				const snapshot = await dapSessionManager.terminate(undefined, timeoutMs);
			...
			case "output": {
				const { snapshot, output } = dapSessionManager.getOutput();
			...
			case "set_breakpoint": {
				if (typeof args.function === "string" && args.function) {
					const result = await dapSessionManager.setFunctionBreakpoint(
						args.function,
				...
				if (typeof args.file !== "string" || typeof args.line !== "number") {
					throw new Error("'file' and 'line' are required for file breakpoints (or use 'function' for function breakpoints).");
				}
				const result = await dapSessionManager.setBreakpoint(
					args.file,
					args.line,
					typeof args.condition === "string" ? args.condition : undefined,
					undefined,
			...
			case "remove_breakpoint": {
				if (typeof args.function === "string" && args.function) {
					const result = await dapSessionManager.removeFunctionBreakpoint(
						args.function,
						undefined,
						timeoutMs,
					);
				...
				const result = await dapSessionManager.removeBreakpoint(
					args.file,
					args.line,
					undefined,
					timeoutMs,
			...
			case "set_instruction_breakpoint": {
				...
				const result = await dapSessionManager.setInstructionBreakpoint(
			...
			case "remove_instruction_breakpoint": {
				...
				const result = await dapSessionManager.removeInstructionBreakpoint(
					args.instruction_reference,
					typeof args.offset === "number" ? args.offset : undefined,
					undefined,
					timeoutMs,
			...
			case "data_breakpoint_info": {
				...
				const result = await dapSessionManager.dataBreakpointInfo(
					args.name,
			...
			case "set_data_breakpoint": {
				...
				const result = await dapSessionManager.setDataBreakpoint(
					args.data_id,
					args.access_type,
					typeof args.condition === "string" ? args.condition : undefined,
					typeof args.hit_condition === "string" ? args.hit_condition : undefined,
			...
			case "remove_data_breakpoint": {
				...
				const result = await dapSessionManager.removeDataBreakpoint(
					args.data_id,
					undefined,
					timeoutMs,
				);
			...
			// ── Execution control ───────────────────────────────
			case "continue": {
				const outcome = await dapSessionManager.continue(undefined, timeoutMs);
				return buildOutcomeText(outcome, timeout, "Continue");
			}
			case "step_over": {
				const outcome = await dapSessionManager.stepOver(undefined, timeoutMs);
				return buildOutcomeText(outcome, timeout, "Step over");
			}
			case "step_in": {
				const outcome = await dapSessionManager.stepIn(undefined, timeoutMs);
				return buildOutcomeText(outcome, timeout, "Step in");
			}
			case "step_out": {
				const outcome = await dapSessionManager.stepOut(undefined, timeoutMs);
				return buildOutcomeText(outcome, timeout, "Step out");
			}
			case "pause": {
				const snapshot = await dapSessionManager.pause(undefined, timeoutMs);
			...
			case "evaluate": {
				...
				const ctx = args.context ?? "repl";
				const result = await dapSessionManager.evaluate(
					args.expression,
					ctx as "watch" | "repl" | "hover" | "clipboard" | "variables",
					typeof args.frame_id === "number" ? args.frame_id : undefined,
					undefined,
					timeoutMs,
				);
			...
			case "stack_trace": {
				const result = await dapSessionManager.stackTrace(
			...
			case "threads": {
				const result = await dapSessionManager.threads(undefined, timeoutMs);
				return formatThreads(result.threads);
			}
			case "scopes": {
				const result = await dapSessionManager.scopes(
					typeof args.frame_id === "number" ? args.frame_id : undefined,
					undefined,
					timeoutMs,
				);
			...
			case "variables": {
				...
				const result = await dapSessionManager.variables(variableRef, undefined, timeoutMs);
			...
			case "disassemble": {
				...
				const result = await dapSessionManager.disassemble(
			...
			case "read_memory": {
				...
				const result = await dapSessionManager.readMemory(
			...
			case "write_memory": {
				...
				const result = await dapSessionManager.writeMemory(
					args.memory_reference,
					args.data,
					typeof args.offset === "number" ? args.offset : undefined,
					args.allow_partial,
			...
			case "modules": {
				...
				const result = await dapSessionManager.modules(
			...
			case "loaded_sources": {
				...
				const result = await dapSessionManager.loadedSources(undefined, timeoutMs);
			...
			case "custom_request": {
				...
				const result = await dapSessionManager.customRequest(
					args.command,
					customArgs,
					undefined,
					timeoutMs,
